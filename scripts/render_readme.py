#!/usr/bin/env python3
"""Render a README with the latest crates list."""
...
New crates published between {start} and {end}.
...
def crate_link(name):
    url = "https://crates.io/crates/" + urllib.parse.quote(name)
    return f"[{name}]({url})"
...
def render_rows(rows):
    return "\n".join(
        f"| {display_time(row.get('created_at'))} | {crate_link(row.get('crate', ''))} "
        f"| {clean_cell(row.get('version'), 20)} | {display_count(row.get('downloads'))} "
...
def render_section(entry, rows, limit):
    ...
    if rows is None:
        ...
        body = "_No crates were published in this window._"
    else:
        body = TABLE_HEADER + "\n" + render_rows(rows[:limit])
        if len(rows) > limit:
            body += (
                f"\n\n_Showing the first {limit:,} of {len(rows):,} crates; "
